...
| Export | Contents |
|--------|----------|
| `@cowboylogic/cerebro-schema` | TypeScript types: `CerebroCatalog`, `Artifact`, `ArtifactSet`, `ArtifactType`, `ToolId`, `Scope` |
| `@cowboylogic/cerebro-schema/validate` | AJV-based `validateCatalog()`, `isCerebroCatalog()`, `ValidationResult` (includes AJV runtime) |
...
```bash
npm install @cowboylogic/cerebro-schema
```
...
import type { CerebroCatalog, Artifact } from '@cowboylogic/cerebro-schema';
